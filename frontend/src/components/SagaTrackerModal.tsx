...
export const SagaTrackerModal: React.FC<SagaTrackerModalProps> = ({
  orderId,
  onClose,
}) => {
          ...
          >
            ...
            <div>
              <h2 style={{ fontSize: '1.35rem' }}>
                Distributed Saga Orchestration Tracker
              </h2>
              <div
                style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}
              >
                Real-time event lifecycle across Order, Inventory, Payment, and
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>
              $
              {(
                (order?.total_amount_cents || order?.totalAmountCents || 0) /
                100
              ).toFixed(2)}
            </div>
          </div>
          <div>
            ...
            >
              {order?.correlation_id
                ? `${order.correlation_id.slice(0, 8)}...`
                : 'Generating...'}
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div style={{ flexGrow: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>
                1. Order Created & Emitted (order.created)
              </div>
              <div
                style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}
              >
                Transactional Outbox in Order DB (Port 3003) published event to
                AWS SNS topic
              </div>
            </div>
          ...
          >
            ...
            <span
              className={`badge ${isInventoryFailure ? 'badge-rose' : isPaymentFailure ? 'badge-amber' : isCompleted || isConfirmed ? 'badge-emerald' : 'badge-amber'}`}
            >
              {isInventoryFailure
                ? 'RESERVATION FAILED'
                : isPaymentFailure
          ...
          >
            ...
            <span
              className={`badge ${isPaymentFailure ? 'badge-rose' : isInventoryFailure ? 'badge-secondary' : isCompleted ? 'badge-emerald' : 'badge-amber'}`}
            >
              {isPaymentFailure
                ? 'DECLINED'
                : isInventoryFailure
          ...
          >
            ...
            <div style={{ flexGrow: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>
                {isCancelled
                  ? '4. Saga Compensation Finished (CANCELLED)'
                  : isCompleted
                    ? '4. Saga Orchestration Complete (COMPLETED)'
                    : '4. Finalizing Order Lifecycle...'}
              </div>
              <div
                style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}
              >
            ...
            <span
              className={`badge ${isCancelled ? 'badge-rose' : isCompleted ? 'badge-emerald' : 'badge-indigo'}`}
            >
              {isCancelled
                ? 'CANCELLED'
                : isCompleted
