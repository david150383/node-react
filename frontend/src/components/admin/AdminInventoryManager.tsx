...
export const AdminInventoryManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([prodsRes, invRes]) => {
        if (!cancelled) {
          setProducts(prodsRes.data || []);
          setTotalItems(
            prodsRes.pagination?.total ?? (prodsRes.data?.length || 0),
          );

          const invLookup: Record<string, InventoryRecord> = {};
          (invRes.data || []).forEach((inv: InventoryRecord) => {
            invLookup[inv.product_id] = inv;
          });
          setInventoryMap(invLookup);
        }
        ...
        {isLoading ? (
          ...
          >
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
              Querying stock levels from Inventory Service (:3004)...
            </p>
          </div>
        ) : products.length === 0 ? (
          <div
                  ...
                  >
                    ...
                    <td
                      style={{
                        padding: '1rem',
                        fontFamily: 'var(--font-mono)',
                        color: reserved > 0 ? '#38bdf8' : 'var(--text-muted)',
                      }}
                    >
                      {reserved} reserved
                    </td>
