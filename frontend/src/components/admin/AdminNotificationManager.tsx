...
export const AdminNotificationManager: React.FC = () => {
  ...
  const [subject, setSubject] = useState(
    'System Broadcast: Platform Maintenance',
  );
  const [body, setBody] = useState(
    'All microservices and database nodes are performing nominal operations.',
  );
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    ...
    try {
      await adminApi.sendManualNotification({
        channel,
        recipient,
        subject,
        body,
          ...
          {channels.map((ch) => (
            <button
              ...
              style={{
                ...
                color:
                  channelFilter === ch ? '#ffffff' : 'var(--text-secondary)',
                border: '1px solid',
                borderColor:
                  channelFilter === ch ? '#6366f1' : 'var(--border-subtle)',
              }}
            >
              {ch}
            </button>
        ...
        {isLoading ? (
          ...
          >
            ...
            <p style={{ color: 'var(--text-secondary)' }}>
              Loading audit stream from Notification Service (:8006)...
            </p>
          </div>
            ...
            <tbody>
              {notifications.map((n) => (
                ...
                >
                  ...
                  <td style={{ padding: '1rem' }}>
                    <span
                      className={`badge ${n.status === 'DELIVERED' ? 'badge-emerald' : 'badge-rose'}`}
                    >
                      {n.status === 'DELIVERED' ? (
              ...
              >
                <div>
                  ...
                  <select
                    value={channel}
                    onChange={(e) =>
                      setChannel(e.target.value as 'EMAIL' | 'SMS' | 'WEBHOOK')
                    }
                    style={{
                      width: '100%',
